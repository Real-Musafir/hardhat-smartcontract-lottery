...
export function serialize(transaction: ethers.providers.TransactionRequest, ecdsaSignature?: SignatureLike) {
    if (transaction.customData == null && transaction.type != EIP712_TX_TYPE) {
        return utils.serializeTransaction(transaction as ethers.PopulatedTransaction, ecdsaSignature);
    }
    ...
    if (ecdsaSignature) {
        const sig = utils.splitSignature(ecdsaSignature);
